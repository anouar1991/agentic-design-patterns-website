...
### Lessons Learned (T-1590)
- [T-1590] Profile page audit — layout, stats display, chapter progress map, and achievements verified in both light/dark modes
- [T-1590] **Unauthenticated state**: Profile page uses `<Navigate to="/" replace />` to silently redirect unauthenticated users to the homepage. No dedicated "please sign in" prompt shown — redirect is instant. Screenshot captured at `profile-unauthenticated-redirect.png`.
- [T-1590] **Light mode layout** (`profile-light-fullpage.png`): Four sections render cleanly — (1) Header card with gradient avatar, "Learner" display name, edit pencil, "1/21 Chapters" emerald badge, Day Streak/Quizzes Passed stats, country selector; (2) Learning Progress with 4 stat cards (Chapters Completed, Quiz Attempts, Average Quiz Score, Perfect Scores) and gradient progress bar at 5%; (3) Chapter Progress Map as 7-column grid showing all 21 chapters with chapter 1 highlighted in emerald; (4) Achievements with 11 badges in 4-column grid, "First Steps" earned with green check.
- [T-1590] **Dark mode layout** (`profile-dark-fullpage.png`): All sections render with proper dark theme contrast. Glass morphism cards have correct border/backdrop effects. Progress bar gradient visible. Achievement badges have appropriate grayscale for unearned vs color for earned.
- [T-1590] **Chapter Progress Map rendering**: 21 tiles in `grid-cols-3 sm:grid-cols-7` layout. Completed chapter 1 shows emerald background with CheckCircle2 icon. Remaining chapters show dark-800 with dark-700 border. Legend shows "Completed (1)" and "Remaining (20)" correctly.
- [T-1590] **Stats cards alignment**: 4-stat grid (`grid md:grid-cols-4 gap-4`) is evenly spaced in both modes. Numbers are `text-3xl font-bold text-dark-50` with `text-sm text-dark-400` labels below.
...
### Lessons Learned (T-1630)
- [T-1630] Cross-chapter consistency audit compared chapters 1 (Prompt Chaining, #f59e0b amber), 5 (Tool Use, #f97316 orange), and 11 (Goal Setting, #f43f5e rose) across header, breadcrumb, tutorial, code blocks, diagrams, quiz, and footer in both light and dark modes
- [T-1630] **Header/Breadcrumb**: Fully consistent across all 3 chapters — compact header with logo, breadcrumb using chapter accent color, search bar, progress counter (1/21), theme toggle, language switcher, GitHub link. Below-header breadcrumb (Home / Chapters / Ch X) identical.
- [T-1630] **Chapter Title Area**: Consistent structure — chapter icon (accent-colored), "Ch X" badge, reading time, completion status, part name, title h1, description paragraph, "The Big Picture" narrative intro card with left accent border.
- [T-1630] **Tutorial Section**: Step numbers use chapter accent color consistently. Callout types (PRO TIP green, WATCH OUT amber, CHECKPOINT blue/teal, Try It Yourself purple) use the same color scheme independent of chapter accent — correct behavior.
- [T-1630] **Code Blocks**: All chapters use dark code theme in both light and dark modes. PYTHON/BASH language badges consistent. Copy button present. Line numbers rendered identically.
- [T-1630] **Minor inconsistency — Clickable code terms**: Ch1 and Ch5 tutorial code blocks have interactive clickable terms ("Click highlighted terms for explanations" label + dashed-underline buttons like "Learn about ChatOpenAI"). Ch11 tutorial code blocks do NOT have clickable terms — they render as plain code without the interactive hint label. This is a content gap (missing codeTerms definitions for Ch11), not a component bug.
- [T-1630] **Diagrams**: All use "Pattern Flow" heading with "Click nodes to explore" subtitle, fullscreen button, zoom controls (zoom in/out + fit). Node colors correctly map: gray for input role, chapter accent for process role, green for output role. Ch1 linear flow, Ch5 fan-out flow, Ch11 cyclic flow — layout differences are appropriate per pattern.
...
### Lessons Learned (T-1720)
- [T-1720] When a module exports a nullable singleton (`SupabaseClient | null`), TypeScript's control-flow narrowing doesn't propagate across function boundaries. Calling `isSupabaseConfigured()` (which checks the same condition) followed by an early return doesn't narrow `supabase` to non-null — the non-null assertion `!` is needed on every access after the guard.
- [T-1720] The `AuthContext.tsx` already used the `supabase!` pattern correctly, but 5 other files (`progressSync.ts`, `useLeaderboard.ts`, `useProfileStats.ts`, `useQuizAttempts.ts`, `usePresence.ts`, `progressMerge.ts`) did not, causing 25 TS18047 build errors. The fix was mechanical: add `!` after `supabase` in all guarded code paths.
- [T-1720] `npx tsc --noEmit` (root `tsconfig.json`) passes even when `npm run build` fails, because the root config has `"files": []` and delegates to `tsconfig.app.json`/`tsconfig.node.json`. Always use `npm run build` for validation, not `npx tsc --noEmit` from root.

### Lessons Learned (T-1730)
- [T-1730] `vite-plugin-pwa` automatically inherits Vite's `base` config for SW registration URL, manifest scope/start_url, and precache manifest paths. No manual `import.meta.env.BASE_URL` usage is needed in `swRegister.ts` — the virtual module `virtual:pwa-register` handles it internally.
- [T-1730] Workbox precache manifest URLs are relative to the SW's location, not the origin. When SW is at `/Agentic_Design_Patterns/sw.js`, relative URL `assets/index.js` correctly resolves to `/Agentic_Design_Patterns/assets/index.js`. No base path prefix is needed in the manifest itself.
- [T-1730] Adding `navigateFallbackAllowlist: [/^\/Agentic_Design_Patterns\//]` to workbox config prevents the SW from intercepting navigation requests to other paths on the same GitHub Pages origin (e.g., other repos). Without it, all navigation requests within scope get the cached `index.html`.

### Lessons Learned (T-1760)
- [T-1760] `vite-plugin-compression` console output shows full absolute paths (`dist//home/noreddine/.../assets/file.js.gz`) in its log messages, but the actual files are correctly placed in `dist/assets/` alongside the originals. This is a cosmetic logging issue in the plugin, not a misplacement bug.
- [T-1760] `localhost` references in production bundles fall into 3 safe categories: (1) library internals using `localhost` as URL parsing defaults (React, i18next), (2) SDK fallback URLs gated by configuration checks (Supabase), and (3) tutorial code example strings in data files. None represent broken production references.
- [T-1760] Every JS/CSS file above the 1024-byte `threshold` configured in `vite-plugin-compression` gets both `.gz` and `.br` companions. Files below threshold are excluded from compression since the overhead would exceed savings — verify the threshold matches your smallest asset if you need universal compression.

### Lessons Learned (T-1780)
- [T-1780] SPAs need both static meta tags (in `index.html` for crawlers that don't execute JS) and dynamic meta tags (via `useDocumentMeta` for client-side navigation). Most social media bots and search crawlers read only the initial HTML response.
...
### Lessons Learned (T-1800)
- [T-1800] **Deployment checkpoint verification**: A full deployment readiness check should verify 6 artifact categories: (1) index.html with SPA redirect handler, (2) 404.html with path-encoding redirect, (3) robots.txt with sitemap reference, (4) sitemap.xml with all routes, (5) sw.js service worker, (6) manifest.webmanifest for PWA.
- [T-1800] **SPA 404 redirect cannot be fully tested locally**: Vite's preview server handles SPA routing internally (falls back to index.html), so it doesn't trigger 404.html. The 404→redirect→replaceState chain can only be verified by code inspection locally. Full E2E testing requires actual GitHub Pages deployment.
- [T-1800] **Manual GitHub setup required**: After the first push to `main`, one manual step is needed: go to repo Settings → Pages → select `gh-pages` branch as source. This is a one-time setup that cannot be automated via the workflow. The workflow creates the `gh-pages` branch automatically on first run via `peaceiris/actions-gh-pages`.
- [T-1800] **Zero console errors across all routes**: Production build with Vite 7 produces clean output with no runtime warnings. The Supabase graceful degradation (T-1720) prevents auth-related errors when env vars are missing.

### Lessons Learned (T-1810)
- [T-1810] **Deployment docs need activation AND workflow steps**: GitHub Pages requires two distinct phases: (1) the CI workflow that builds and pushes to `gh-pages`, and (2) manual repo settings to enable Pages from that branch. Documenting only the workflow leads to "deployed but not live" confusion.
- [T-1810] **Troubleshooting section prevents repeat support requests**: The five most common GitHub Pages issues (blank page, 404 on refresh, missing assets, workflow failures, Supabase not working) each have distinct root causes that are non-obvious to first-time deployers. Documenting these proactively saves significant debugging time.
